...
class AnomalyController
{
    ...
    paint()
    {
        if(gravity_sphere._scale_loaded)
        {
            let anomaly_outside_rad = new Circle(anomaly_ctx, 
                                                    (anomaly_canvas.width/2) - Math.trunc(gravity_sphere.rad * gravity_sphere.d_scale), 
                                                    (anomaly_canvas.height/2) - Math.trunc(gravity_sphere.rad * gravity_sphere.d_scale), 
                                                    Math.trunc(gravity_sphere.rad * gravity_sphere.d_scale * 2),
                                                    "#FF0000");

            let anomaly_inside_rad = new Circle(anomaly_ctx, 
                                                    (anomaly_canvas.width/2) - Math.trunc(gravity_sphere.rad * gravity_sphere.x_scale), 
                                                    (anomaly_canvas.height/2) - Math.trunc(gravity_sphere.rad * gravity_sphere.x_scale), 
                                                    Math.trunc(gravity_sphere.rad * gravity_sphere.x_scale * 2),
                                                    "#000000");
            ...
            anomaly_ctx.fillText("Depth: "+gravity_sphere.depth.toFixed(1)+" m", 5, 12);
            anomaly_ctx.fillText("Radius: "+gravity_sphere.rad.toFixed(1)+" m", 5, 24);
    ...
    changeValues()
    {
        if(this.left_arrow_down)
        {
            if(gravity_sphere.rad > 0.6)
                gravity_sphere.rad -= 0.1;
        }
        else if(this.right_arrow_down)
        {
            if(gravity_sphere.rad < gravity_sphere.depth)
                gravity_sphere.rad += 0.1;
        }
        else if(this.up_arrow_down)
        {
            if(gravity_sphere.rad < gravity_sphere.depth-0.1)
                gravity_sphere.depth -= 0.1;
        }
        else if(this.down_arrow_down)
        {
            if(gravity_sphere.depth < 24)
                gravity_sphere.depth += 0.1;
        }
        gravity_sphere.paint();
        gravity_sphere.setTableValues();
    }
...
{
    gravity_sphere.x_loc = parseFloat(this.value);
    gravity_sphere.paint();
    gravity_sphere.setTableValues();
};
function moveAnomLeft()
{
    if(gravity_sphere.x_loc > gravity_sphere.x_min)
    {
        anom_slider.value--;
        gravity_sphere.x_loc -= 1
        gravity_sphere.paint();
        gravity_sphere.setTableValues();
    }
}
...
{
    if(gravity_sphere.x_loc < gravity_sphere.x_max)
    {
        anom_slider.value++;
        gravity_sphere.x_loc += 1
        gravity_sphere.paint();
...
{
    gravity_sphere.rho = gravity_sphere.rhoFormat(this.value, true)
    gravity_sphere.paint();
    gravity_sphere.setTableValues();
};
...
{
    rho_slider.value--;
    gravity_sphere.rho = gravity_sphere.rhoFormat(rho_slider.value, true);
    gravity_sphere.paint();
    gravity_sphere.setTableValues();
}
...
{
    gravity_sphere.bias = gravity_sphere.biasFormat(this.value, true);
    gravity_sphere.paint();
    gravity_sphere.setTableValues();
}
...
{
    bias_slider.value--;
    gravity_sphere.bias = gravity_sphere.biasFormat(bias_slider.value, true);
    gravity_sphere.paint();
    gravity_sphere.setTableValues();
...
file_input.onchange = function ()
{
    ...
    reader.onload = function(progressEvent)
    {
        ...
        {
            row = rows[i].split(",");
            gravity_sphere.x_data.push(parseInt(row[0]));
            gravity_sphere.g_data.push(parseFloat(row[1]));
        }
        gravity_sphere.n_data = gravity_sphere.x_data.length;
        gravity_sphere.setScaleLoaded(true);
        enableWidgets();
        anomaly_ctx.clearRect(0,0, anomaly_canvas.width, anomaly_canvas.height);
...
{
    let progress = time_stamp - last_render;
