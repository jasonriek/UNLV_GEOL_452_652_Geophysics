...
const canvas = document.getElementById("GravityCylinder");
...
class GravityCylinder
{
    ...
    {
        let save = null;
        save = ( 2.0 * G * Math.PI * Math.pow(this.rad, 2.0) * this.depth *
        this.rho / ((x - this.x_loc) * (x - this.x_loc) + this.depth * this.depth) 
...
let gravity_cylinder = new GravityCylinder();
gravity_cylinder.start();

// LOAD THE TREE
gravity_cylinder.tree.onload = function()
{
    ctx.drawImage(gravity_cylinder.tree, 50, 238);
};
gravity_cylinder.tree.src = "../../../images/tree.png";
