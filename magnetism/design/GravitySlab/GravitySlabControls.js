...
class AnomalyController
{
    ...
    paint()
    {
        if(gravity_slab._scale_loaded)
        {
        
            anomaly_ctx.clearRect(0, 0, anomaly_canvas.width, anomaly_canvas.height);

            anomaly_ctx.fillStyle = "#000000";
            anomaly_ctx.fillText("Depth: "+(-gravity_slab.top).toFixed(1)+" m", 5, 12);
            anomaly_ctx.fillText("Width: "+gravity_slab.width.toFixed(1)+" m", 5, 24);
            anomaly_ctx.fillText("Move to Surface ", 83, 19);
            anomaly_ctx.fillText("Move to Deeper Depth", 70, 227);
            anomaly_ctx.fillText("Decrease Width", 9, 87);
            anomaly_ctx.fillText("Increase Width", 159, 87);
    ...
    changeValues()
    {
        if(this.left_arrow_down)
        {
            if(gravity_slab.width >= 5.0)
                gravity_slab.width -= 0.1;
        }
        else if(this.right_arrow_down)
        {
            if(gravity_slab.width <= 500)
                gravity_slab.width += 0.1;
        }
        else if(this.up_arrow_down)
        {
            if(gravity_slab.top <= -0.4)
                gravity_slab.top += 0.1;
        }
        else if(this.down_arrow_down)
        {
            if(gravity_slab.top > -22.7)
                gravity_slab.top -= 0.1;
        }
        gravity_slab.paint();
        gravity_slab.setTableValues();
    }
...
{
    gravity_slab.x_loc = parseFloat(this.value);
    gravity_slab.paint();
    gravity_slab.setTableValues();
};
function moveAnomLeft()
{
    if(gravity_slab.x_loc > gravity_slab.x_min)
    {
        anom_slider.value--;
        gravity_slab.x_loc -= 1
        gravity_slab.paint();
        gravity_slab.setTableValues();
    }
}
...
{
    if(gravity_slab.x_loc < gravity_slab.x_max)
    {
        anom_slider.value++;
        gravity_slab.x_loc += 1
        gravity_slab.paint();
...
{
    gravity_slab.rho = gravity_slab.rhoFormat(this.value, true)
    gravity_slab.paint();
    gravity_slab.setTableValues();
};
...
{
    rho_slider.value--;
    gravity_slab.rho = gravity_slab.rhoFormat(rho_slider.value, true);
    gravity_slab.paint();
    gravity_slab.setTableValues();
}
...
{
    gravity_slab.bias = gravity_slab.biasFormat(this.value, true);
    gravity_slab.paint();
    gravity_slab.setTableValues();
}
...
{
    bias_slider.value--;
    gravity_slab.bias = gravity_slab.biasFormat(bias_slider.value, true);
    gravity_slab.paint();
    gravity_slab.setTableValues();
...
file_input.onchange = function ()
{
    ...
    reader.onload = function(progressEvent)
    {
        ...
        {
            row = rows[i].split(",");
            gravity_slab.x_data.push(parseInt(row[0]));
            gravity_slab.g_data.push(parseFloat(row[1]));
        }
        gravity_slab.n_data = gravity_slab.x_data.length;
        gravity_slab.setScaleLoaded(true);
        enableWidgets();
        anomaly_ctx.clearRect(0,0, anomaly_canvas.width, anomaly_canvas.height);
