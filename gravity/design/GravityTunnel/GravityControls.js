...
class GravityControls
{
    ...
    //that must be integers
        ...
        this.DxBarMax=Math.trunc((this.DxMax-this.DxMin)/this.dDx+2.1)-this.DxMin; //Maximum Scroll Bar Value
        this.RhoMin=-3.2; //gm/cm^3
        this.RhoMax=0.0;
        this.dRho=0.05;
        this.RhoBarMin=1;
        this.RhoBarMax=Math.trunc((this.RhoMax-this.RhoMin)/this.dRho+2.1)-0.05;
        ...
        this.dN=1;
        this.NBarMin=1;
        this.NBarMax=Math.trunc((this.NMax-this.NMin)/this.dN+0.5)-1;
        this.s_value;	//variable used to collect scrollbar info
        this.s_size;   //variable to hold scroller size
    ...
    {
        //Set Station Spacing
        this.s_value = Math.trunc((gravity_tunnel.dx - this.DxMin) * (this.DxBarMax - this.DxBarMin) /
        ...
        this.s_value = Math.trunc((gravity_tunnel.rho - this.RhoMin) * (this.RhoBarMax - this.RhoBarMin) /
        ...
        this.s_value = Math.trunc((gravity_tunnel.ndata-this.NMin) * (this.NBarMax - this.NBarMin) /
        ...
        this.s_value = Math.trunc((gravity_tunnel.std-this.StdMin) * (this.StdBarMax - this.StdBarMin) /
...
{
    gravity_tunnel.dx = gravity_controls.dxfFormat(this.value, true);
...
{
    gravity_tunnel.ndata = this.value;
    gravity_controls.frameChanged();
};

std_dev_slider.oninput = function ()
{
    gravity_tunnel.std = gravity_controls.stdFormat(this.value, true);
    gravity_controls.frameChanged();
};
...
function n_of_obs_LeftButton()
{
    ...
    gravity_tunnel.ndata = n_of_obs_slider.value;
    gravity_controls.frameChanged();
}
...
{
    dxf_slider.value--;
    gravity_tunnel.dx = gravity_controls.dxfFormat(dxf_slider.value, true);
    gravity_controls.frameChanged();
}
...
function std_LeftButton()
{
    ...
    gravity_tunnel.std = gravity_controls.stdFormat(std_dev_slider.value, true);
    gravity_controls.frameChanged();
}
